--[[
    CDECAD Sync - 911 & NPC Reports Client
    ...
    CreateThread(function()
        while true do
            ...
            if vehicle ~= 0 then
                ...
                -- Check if player is stealing a vehicle (not their own, breaking in)
                if not trackedVehicles[vehicleId] then
                    local isEntering = GetIsTaskActive(playerPed, 160) -- TASK_ENTER_VEHICLE
                    local plate = GetVehicleNumberPlateText(vehicle)
                    
                    -- Simple check: player just entered a locked vehicle they don't own
                    -- This is a simplified version - you might want to integrate with your vehicle key system
...
if Config.NPCReports.Enabled and Config.NPCReports.SpeedCamera and Config.NPCReports.SpeedCamera.Enabled then
    -- Speed camera locations (you can customize these)
    local SpeedCameras = {
...
RegisterCommand('panic', function()
    ...
    -- Check if player is LEO/EMS (optional - you might want to restrict this)
    local PlayerData = exports.qbx_core:GetPlayerData()
    local job = PlayerData and PlayerData.job
    
    if not job then return end
    
    local callerInfo = string.format('%s %s (%s)', 
...
print('[CDECAD-SYNC] 911 Client script loaded')
