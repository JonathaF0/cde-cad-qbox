...
-- Format date from QBox format (YYYY-MM-DD) to CAD format
function Utils.FormatDate(dateStr)
    if not dateStr then return nil end
    -- QBox typically stores as "1990-01-15" or similar
    return dateStr
end

-- Convert QBox gender to CAD gender
...
function Utils.ValidateCitizenId(citizenid)
    if not citizenid then return false end
    -- QBox citizenid is typically 8 characters alphanumeric
    return string.len(citizenid) >= 5
end
